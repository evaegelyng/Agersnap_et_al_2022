...
libraries = [x for x in glob("data/X201SC20011262-Z01-F003/raw_data/*") if os.path.isdir(x)]
    ...
    with open(os.path.join(library_root, "tags.txt")) as tags_file:
        for line in tags_file:
            ...
            gwf.target(
                ...
                mkdir -p {folderAS}
                mkdir -p {folderSS}
                if grep -q "Empty file  {inputASF}\|Empty file  {inputASR}" ".gwf/logs/sickle_{project_name}_{library_id}_{tag_id}.stdout"
                then
                 echo "" > {outputASF}
                 echo "" > {outputASR}
                fi
                if grep -q "Empty file  {inputSSF}\|Empty file  {inputSSR}" ".gwf/logs/sickle_{project_name}_{library_id}_{tag_id}.stdout"
                then
                 echo "" > {outputSSF}
                 echo "" > {outputSSR}
                fi 
...
def taxonomy(fileName, taxonomyFolder, blastFolder, k):
    ...
    then
      Rscript scripts/taxonomy_fall.r {inputFile} {outputFile}
    else
      touch {outputFile}
    fi
